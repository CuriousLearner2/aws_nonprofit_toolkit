...
def initialize(repo, contract, campaign_id="campaign-01"):
    ...
    return campaign.campaign_ledger_init(campaign_id, repo, gate, [contract])
...
def test_happy_path_restart_and_deterministic_next(repo, contract, monkeypatch):
    initialize(repo, contract); assert campaign.campaign_ledger_next("campaign-01")["kind"] == "IMPLEMENT"
    commit = commit_change(repo); assert campaign.campaign_ledger_record_result("campaign-01", result(commit))["state"] == "VALIDATING"
    assert campaign.campaign_ledger_next("campaign-01")["kind"] == "VALIDATE"; assert campaign.campaign_ledger_record_result("campaign-01", result(commit))["state"] == "COMMITTED"; assert campaign.campaign_ledger_next("campaign-01") == {"action": "STOP", "state": "COMMITTED"}
...
def test_absent_dirty_and_stale_commands_fail_closed(repo, contract):
    ...
    calls = [lambda: campaign.campaign_ledger_status("missing"), lambda: campaign.campaign_ledger_next("missing"),
             lambda: campaign.campaign_ledger_record_result("missing", result(head)), lambda: campaign.campaign_ledger_quarantine("missing", "x"),
             lambda: campaign.campaign_ledger_stop("missing", "x")]
    for call in calls: fails(call)
    initialize(repo, contract); (repo / "dirty").write_text("x")
    fails(lambda: campaign.campaign_ledger_status("campaign-01"), "dirty"); (repo / "dirty").unlink(); commit_change(repo); fails(lambda: campaign.campaign_ledger_status("campaign-01"), "stale HEAD")
...
def test_contract_gate_and_ledger_mutations_fail_closed(repo, contract, tmp_path):
    ...
    fails(lambda: campaign.campaign_ledger_status("campaign-01"), "contract digest")
...
def test_legacy_path_prompt_state_and_repeated_transitions_are_rejected(repo, contract):
    ...
    fails(lambda: campaign.campaign_initialize("campaign-01", legacy), "disabled"); campaign.campaign_ledger_next("campaign-01"); fails(lambda: campaign.campaign_ledger_record_result("campaign-01", {**result(git(repo, "rev-parse", "HEAD")), "state": "COMMITTED"}), "matching pending"); campaign.campaign_ledger_quarantine("campaign-01", "manual"); fails(lambda: campaign.campaign_ledger_quarantine("campaign-01", "again"), "terminal")


def test_stop_repeat_and_atomic_write_failure(repo, contract, monkeypatch):
    initialize(repo, contract, "campaign-02"); campaign.campaign_ledger_stop("campaign-02", "manual"); fails(lambda: campaign.campaign_ledger_stop("campaign-02", "again"), "terminal")
