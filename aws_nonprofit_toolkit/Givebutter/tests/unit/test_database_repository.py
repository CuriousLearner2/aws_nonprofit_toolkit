...
class TestDatabaseRepositoryMethods:
    ...
    def test_all_repository_protocol_methods_implemented(self):
        ...
        # All protocol methods now implemented:
        ...
        assert callable(repo.get_normalizations)
...
class TestDatabaseGetValidation:
    ...
    def test_get_validation_row_data(self, temp_db_with_validation_data):
        ...
        assert john_row.phone == '(555) 123-4567'
...
class TestDatabaseGetNormalizations:
    """Verify DatabaseImportRepository.get_normalizations() implementation and parity."""

    @pytest.fixture
    def temp_db_with_normalizations_data(self):
        """Create temp database with normalization test data matching NORMALIZATION_SUGGESTIONS fixture."""
        with tempfile.TemporaryDirectory() as tmpdir:
            db_path = Path(tmpdir) / 'normalizations_test.db'
            db_url = f'sqlite:///{db_path}'
            ...
            # Create 6 normalization review items matching NORMALIZATION_SUGGESTIONS fixture
            # NORM-001: John Smith - Name field
            ...
            # NORM-002: Robert Smith - Phone field
            ...
            # NORM-003: Jane Doe - Email field
            norm_item_3 = ReviewItem(
                batch_id='IMP-2025-0101-A',
                item_type='normalization',
                status='pending',
                payload_json={
            ...
            session.add(norm_item_3)

            # NORM-004: Sarah Williams - Address field
            norm_item_4 = ReviewItem(
                batch_id='IMP-2025-0101-A',
                item_type='normalization',
                status='pending',
                payload_json={
            ...
            session.add(norm_item_4)

            # NORM-005: Michael Brown - Name field
            norm_item_5 = ReviewItem(
                batch_id='IMP-2025-0101-A',
                item_type='normalization',
                status='pending',
                payload_json={
            ...
            session.add(norm_item_5)

            # NORM-006: Mary Johnson - Phone field
            norm_item_6 = ReviewItem(
                batch_id='IMP-2025-0101-A',
                item_type='normalization',
                status='pending',
                payload_json={
    ...
    def test_get_normalizations_returns_view_model(self, temp_db_with_normalizations_data):
        """Test that get_normalizations returns NormalizationPageViewModel."""
        repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        result = repo.get_normalizations('IMP-2025-0101-A')

        assert isinstance(result, NormalizationPageViewModel)

    def test_get_normalizations_returns_frozen_view_model_not_orm(self, temp_db_with_normalizations_data):
        """Test that get_normalizations returns frozen dataclass, not ORM object."""
        repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        result = repo.get_normalizations('IMP-2025-0101-A')
    ...
    def test_get_normalizations_has_required_fields(self, temp_db_with_normalizations_data):
        """Test that returned normalizations page has all required fields."""
        repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        result = repo.get_normalizations('IMP-2025-0101-A')
        ...
        assert hasattr(result, 'filename')
        assert hasattr(result, 'progress')
        assert hasattr(result, 'current_suggestion')
        assert hasattr(result, 'current_suggestion_index')
        assert hasattr(result, 'total_suggestions')

    def test_get_normalizations_correct_batch_metadata(self, temp_db_with_normalizations_data):
        """Test that normalizations page returns correct batch metadata."""
        repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        result = repo.get_normalizations('IMP-2025-0101-A')
    ...
    def test_get_normalizations_correct_progress(self, temp_db_with_normalizations_data):
        """Test that normalizations page returns correct progress percentage."""
        repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        result = repo.get_normalizations('IMP-2025-0101-A')
    ...
    def test_get_normalizations_correct_total_suggestions(self, temp_db_with_normalizations_data):
        """Test that normalizations page returns correct total_suggestions count."""
        repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        result = repo.get_normalizations('IMP-2025-0101-A')

        # We seeded 6 normalization items
        assert result.total_suggestions == 6

    def test_get_normalizations_correct_current_suggestion_index(self, temp_db_with_normalizations_data):
        """Test that current_suggestion_index is 1."""
        repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        result = repo.get_normalizations('IMP-2025-0101-A')
    ...
    def test_get_normalizations_current_suggestion_is_first(self, temp_db_with_normalizations_data):
        """Test that current_suggestion is the first normalization item."""
        repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        result = repo.get_normalizations('IMP-2025-0101-A')

        # First item is NORM-001: John Smith - Name
        assert result.current_suggestion.id == 'NORM-001'
        assert result.current_suggestion.contact_name == 'John Smith'
        assert result.current_suggestion.field_name == 'Name'
        assert result.current_suggestion.original_value == 'john smith'
        assert result.current_suggestion.suggested_value == 'John Smith'
        assert result.current_suggestion.normalization_type == 'Capitalization Fix'
        assert result.current_suggestion.status == 'Pending'

    def test_get_normalizations_current_suggestion_structure(self, temp_db_with_normalizations_data):
        """Test that current_suggestion has all required fields."""
        repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        result = repo.get_normalizations('IMP-2025-0101-A')

        suggestion = result.current_suggestion
        assert isinstance(suggestion, NormalizationRow)
        assert hasattr(suggestion, 'id')
        assert hasattr(suggestion, 'contact_name')
        assert hasattr(suggestion, 'field_name')
        assert hasattr(suggestion, 'original_value')
        assert hasattr(suggestion, 'suggested_value')
        assert hasattr(suggestion, 'normalization_type')
        assert hasattr(suggestion, 'status')

    def test_get_normalizations_to_template_dict(self, temp_db_with_normalizations_data):
        """Test that NormalizationPageViewModel converts to template dict correctly."""
        repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        result = repo.get_normalizations('IMP-2025-0101-A')
        template_dict = result.to_template_dict()
        ...
        assert isinstance(template_dict, dict)
        assert 'batch' in template_dict
        assert 'current_suggestion' in template_dict
        assert 'current_suggestion_index' in template_dict
        assert 'total_suggestions' in template_dict
        ...
        # Verify current_suggestion structure
        assert isinstance(template_dict['current_suggestion'], dict)
        assert template_dict['current_suggestion']['id'] == 'NORM-001'

        # Verify navigation state
        assert template_dict['current_suggestion_index'] == 1
        assert template_dict['total_suggestions'] == 6

    def test_get_normalizations_no_database_mutation(self, temp_db_with_normalizations_data):
        """Test that get_normalizations does not mutate database state."""
        engine = create_engine(temp_db_with_normalizations_data)
        Session = sessionmaker(bind=engine)
    ...
    def test_get_normalizations_fixture_parity(self, temp_db_with_normalizations_data):
        """Test complete parity with FixtureImportRepository.get_normalizations()."""
        fixture_result = FixtureImportRepository.get_normalizations('IMP-2025-0101-A')
        db_repo = DatabaseImportRepository(database_url=temp_db_with_normalizations_data)
        db_result = db_repo.get_normalizations('IMP-2025-0101-A')
        ...
        assert fixture_result.current_suggestion_index == db_result.current_suggestion_index
        assert fixture_result.total_suggestions == db_result.total_suggestions

        # Compare current suggestion fields
        assert fixture_result.current_suggestion.id == db_result.current_suggestion.id
        assert fixture_result.current_suggestion.contact_name == db_result.current_suggestion.contact_name
        assert fixture_result.current_suggestion.field_name == db_result.current_suggestion.field_name
        assert fixture_result.current_suggestion.original_value == db_result.current_suggestion.original_value
        assert fixture_result.current_suggestion.suggested_value == db_result.current_suggestion.suggested_value
        assert fixture_result.current_suggestion.normalization_type == db_result.current_suggestion.normalization_type
        assert fixture_result.current_suggestion.status == db_result.current_suggestion.status
...
class TestDatabaseGetDuplicates:
    ...
    def test_get_duplicates_contact_a_fields(self, temp_db_with_duplicates_data):
        ...
        assert contact_a.phone == '(555) 123-4567'
    ...
    def test_get_duplicates_contact_b_fields(self, temp_db_with_duplicates_data):
        ...
        assert contact_b.phone == '(555) 123-4567'
